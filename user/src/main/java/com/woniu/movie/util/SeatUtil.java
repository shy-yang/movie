...
public class SeatUtil {
            ...
            for (int k = 0; k < seatsLine.size(); k++) {
                ...
                for (int j = 1; j <= X; j++) {
                    if (Integer.parseInt(seatsLine.get(k).getSeatX()) == j) {
                        sb.replace(j - 1, j, "c" + "[" + seatsLine.get(k).getSeatId() + "]");
                    }
                }
            }
            //处理完当前行座位后，加入集合中
    ...
    /**
     * 根据MovieTicketVo对象集合生成转换成前端可识别的字符串集合 并找出已售出的座位坐标
     *
     * @param movieTicketVoList 座位对象集合
     * @return 返回已售出座位的movieTicketId集合和所有座位排布集合
     */
    public static Map<String, Object> getMovieTicketSeat(List<MovieTicketVo> movieTicketVoList) {
        List<Seat> seatList = new ArrayList<>();
        List<String> unavailableSeat = new ArrayList<>();
        //获得所有座位排列和所有unavailable座位
        movieTicketVoList.forEach(m -> {
            seatList.add(new Seat(m.getMovieTicketId(), m.getMovieTicketSeatX(), m.getMovieTicketSeatY()));
                    if (!"1".equals(m.getMovieTicketSeatState())) {
                        unavailableSeat.add(m.getMovieTicketId() + "");
                    }
                }
        );
        List<String> allSeat = getSeatSquare(seatList);
        Map<String, Object> map = new HashMap<>();
        map.put("allSeat", allSeat);
        map.put("unavailableSeat", unavailableSeat);
        return map;
    }
}
